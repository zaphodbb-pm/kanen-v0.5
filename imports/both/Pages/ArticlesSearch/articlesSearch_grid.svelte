    ...
    {#each items as row, idx}
        <div class="column">
            ...
            <div class="card" style="overflow: hidden; max-width: 32rem;">
                ...
                <div>
                    <h2 class="is-title-4">
                        <a href='/articlesShow?articleId={row.articleId?.value ?? "na"}' target="_blank">{row.name?.value}</a>
                    </h2>
                    ...
                    <p class="is-size-7">{row.issue?.value} - {row["type.name"]?.value ?? ""}</p>
                </div>
            </div>
