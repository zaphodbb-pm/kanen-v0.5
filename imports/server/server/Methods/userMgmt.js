...
Meteor.methods({
    ...
    /**
     * Check for unique email and then add new subscriber.
     *
    ...
    addSubscriber(email) {
        ...
        const test = Accounts.findUserByEmail(email, {fields: {author: 1} });

        if(test){
            return {status: 406, uid: email, text: `User email ${email} already exists.`};
        }else{

            let subscriberId = Accounts.createUser({
                username: username,
                email: email,
                password: ""
            });
            ...
            if(!subscriberId){
                return {status: 500, _id: email, text: `Failed to create a new subscriber`};
            }
            ...
            let addins = {
                ...
                role: {_id: "standard", name: "Standard"},
            ...
            // @ts-ignore
            Meteor.users.update(subscriberId, {$set: addins}, function (err, res) {
                console.log("createSubscriber", err, res);
            });

            Accounts.sendEnrollmentEmail(subscriberId);

            return {status: 200, _id: email, text: `${username} has been added to subscribers`};
        }
    },

    /**
     * Insert a new subscriber object into collection.
     *
    ...
    createSubscriber(email) {
        ...
        if (Meteor.user() && verifyRole(Meteor.user(), ["administrator"] )) {
            ...
            return {status: 200, _id: subscriberId, text: `${username} has been added to users`};
        }
    },
    ...
    /**
     * Update a user's arbitrary field value.
     *
    ...
    userProfileUpdateItem: function (item, val) {
        check(item, String);
        check(val, Match.OneOf(String, Object, Array) );

        if( Meteor.userId() ) {     // check if user is logged in
            const obj = {...objectify(item, val)};

            Meteor.users.update({_id: Meteor.user()._id}, {$set: obj});
            return {status: 200, _id: Meteor.user()._id, text: `${item} has been updated`};
        }else{
            return {status: 404, _id: null, text: `User not found; ${item} has NOT been updated`};
        }
    },
