...
describe("function fileName", function () {
  data.forEach( (test, idx) => {
    ...
    it(`test ${idx + 1}: ${test.name} + ${test.ext}`, function(){
      const result = fileName(test.name, test.ext);

      // added date part: _2023-02-11#13_13 is 17 characters long
      // must also account for added "." for the extension part
      const dateLength = result.length - test.name.length - test.ext.length - 1 === 17;
